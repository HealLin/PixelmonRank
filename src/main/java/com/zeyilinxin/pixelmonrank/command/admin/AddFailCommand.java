...
public class AddFailCommand {
    ...
    public static boolean onCommand(CommandSender sender, Command cmd, String s, String[] args , PixelmonCommand command) {
        command.pixlemonRank.getBukkitScheduler().runTask(command.pixlemonRank , ()->{
            LogPostUtils.postPlayerLog(sender.getName() , "addFail");
            String name = args[2];
            ...
            new PixelmonStorae(command.pixlemonRank).getStorage().addFail(name ,num );
            sender.sendMessage("§a成功为§6" + name + "§a添加§6" + num + "§a败场");
            sender.sendMessage("§a现在§6" + name + "§a败场为§6" + new PixelmonStorae(command.pixlemonRank).getStorage().getFail(name));
            LogPostUtils.postLog(sender.getName() + "给" + name + "增加了" + num + "败场");
        });
        return true;
    }
}
