...
public class RecoveryPlayerCommand {
    ...
    public static boolean onCommand(CommandSender sender, Command cmd, String s, String[] args , PixelmonCommand command) {
        command.pixlemonRank.getBukkitScheduler().runTask(command.pixlemonRank , ()->{
            LogPostUtils.postPlayerLog(sender.getName() , "recoveryPlayer");
            String name = args[2];
            command.pixlemonRank.getSettingsConfig().recoveryPlayer(name);
            sender.sendMessage("§a成功允许§6" + name + "§a参加排位赛");
            LogPostUtils.postLog(sender.getName() + "恢复了" + name + "参加比赛");
        });
        return true;
    }
}
